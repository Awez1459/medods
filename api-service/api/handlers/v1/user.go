...
import (
	models "medods/api-service/api/models"
	...
	"medods/api-service/internal/pkg/utils"
	valid "medods/api-service/internal/pkg/validation"
	"net/http"
...
func (h *HandlerV1) Create(c *gin.Context) {
	...
	isEmail, err := h.Service.UserService().CheckUniquess(c, &pbu.FV{
		Field: "email",
		Value: body.Email,
	})
	if err != nil {
	...
	response, err := h.Service.UserService().Create(c, &pbu.User{
		Id:           newId,
...
func (h *HandlerV1) Update(c *gin.Context) {
	...
	getUser, err := h.Service.UserService().Get(c, &pbu.Filter{
	...
	response, err := h.Service.UserService().Update(c, &pbu.User{
		Id:          userID,
		FullName:    body.FullName,
		Email:       body.Email,
